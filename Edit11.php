<?php 
    ...
    if (isset($_REQUEST['update_id'])) {
        try {
            ...
            $select_stmt = $db->prepare("SELECT * FROM department WHERE DepartmentID = :id");
    ...
    if (isset($_REQUEST['btn_update'])) {
        ...
        if (empty($lastname_up)) {
            $errorMsg = "Please Enter Lastname";
        } else {
            try {
                if (!isset($errorMsg)) {
                    $update_stmt = $db->prepare("UPDATE department SET DepartmentName = :lname_up  WHERE DepartmentID = :id");
